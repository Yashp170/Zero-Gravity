    ...
    public class PlayerClimbing : MonoBehaviour {
            ...
            if (GrippingClimbable) {
                ...
                if (smoothLocomotion) {
                    if(smoothLocomotion.ControllerType == PlayerControllerType.CharacterController) {
                        ...
                        if(smoothLocomotion.UseZeroGravity)
                        {
                            smoothLocomotion.LastDirection = moveDirection.normalized;
                            smoothLocomotion.Speed = smoothLocomotion.GetComponent<CharacterController>().velocity.magnitude / 2;
                            smoothLocomotion.SpeedCompare = smoothLocomotion.Speed;
                            smoothLocomotion.isLerping = false;
                        }
        ...
        void onGrabbedClimbable() {
            ...
            if (smoothLocomotion) {
                ...
                if (smoothLocomotion.UseZeroGravity)
                {
                    smoothLocomotion.Speed = 0;
                    smoothLocomotion.SpeedCompare = 0;
                    smoothLocomotion.isLerping = false;
                }
            }
