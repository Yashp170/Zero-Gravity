...
namespace BNG
{
    ...
    public class SmoothLocomotion : MonoBehaviour
    {
        ...
        public bool StopAfterSomeTime = false;
        public float SlowDownTime = .1f;
        public Vector3 LastDirection;
        public bool isLerping;
        public float Speed = 0;
        public float SpeedCompare = 0;
        ...
        public virtual void MoveCharacter()
        {
            ...
            if (!UseZeroGravity)
            {
                ...
                GetComponent<PlayerGravity>().GravityEnabled = true;
            }
            else
            {
            ...
            if (moveDirection != Vector3.zero)
            {
                ...
                if (UseZeroGravity)
                {
                    ...
                    isLerping = false;
                    StopCoroutine(_StopAfterSomeTime());
                }
            }
            else if (UseZeroGravity && LastDirection != Vector3.zero)
            {
                MoveCharacter(LastDirection * Time.deltaTime * Speed);
                
                if (Speed == SpeedCompare && StopAfterSomeTime && !isLerping)
                {
                    isLerping = true;
                    StartCoroutine(_StopAfterSomeTime());
                }
            }
        }

        public IEnumerator _StopAfterSomeTime()
        {
            while (Speed > 0 && isLerping)
            {
                Speed -= Time.deltaTime * (SlowDownTime / 2 * MovementSpeed);
                yield return null;
            }
